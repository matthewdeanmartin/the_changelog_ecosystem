...
- **Ecosystem sub-pages** (`content/pages/python.md`, `nodejs.md`, etc.)  
  listing all tools for that ecosystem with mini-summaries (COMPLETE — generated by `generate_pages.py`)
- A **Verdicts** page summarising all Recommended / Situational / Avoid ratings

### 3.5 — Tag System (COMPLETE)

Each review article carries a `Tags:` Pelican metadata field. Tags are generated by
`generate_review_stubs.py` from the tool's `_toml_capabilities` list and ecosystem.
Pelican renders `/tags/index.html` and `/tag/{slug}/index.html` for browsing.

Key tag dimensions (deal-breaker filters for users):

| Dimension | Tags |
|-----------|------|
| Format standard | `keep-a-changelog`, `conventional-commits` |
| Versioning | `semantic-versioning` |
| Change workflow | `news-fragments` (fragment assembly), `git-tags`, `autocreate-from-commits` |
| Platform | `github-integration`, `gitlab-integration`, `ci-cd` |
| Scale | `monorepo` |
| Ecosystem | `python`, `rust`, `go`, `node`, `java`, `ruby`, `dotnet`, `cpp` |
